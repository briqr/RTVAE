...
class Encoder_TRANSFORMER(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        self.ff_size = ff_size * ff_size_factor
    ...
    def forward(self, batch):
        ...
        if self.ablation == "average_encoder":
            ...
            mu_query = self.muQuery[y][None]
            sig_query = self.sigmaQuery[y][None]
            xseq = torch.cat((mu_query, sig_query, x), axis=0)
...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        if mask is not None:
            ...
            #print('time queries.shape', timequeries.shape)
            output = self.seqTransDecoder(tgt=timequeries, memory=z) #todo tgt_key_padding_mask=~mask)
