...
#         self.losses = list(self.lambdas) + ["mixed"]
#         if kwargs['dataset'] == 'prox':
#             self.rotation2xyz = Rotation2xyz(device=self.device, is_smplx=True)
#         else:
...
#         #wherenan = torch.where(torch.isnan(batch['output'].detach()))
#         #batch['mask'][wherenan[0]] = False
...
#     @staticmethod
#     def lengths_to_mask(lengths):
#         max_len = lengths.max()#max(lengths)
#         if isinstance(max_len, torch.Tensor):
#             max_len = max_len.item()
#         index = torch.arange(max_len, device=lengths.device).expand(len(lengths), max_len)
#         #mask = index < lengths.unsqueeze(1) todo
#         mask = index <lengths[0][0]
#         return mask
...
#         if True:#todo#n for CAE multi set to False, else True
#             y = classes.to(self.device).repeat(nspa)  # (view(nspa, nats))
#         else:
#             y = classes.to(self.device).unsqueeze(2).expand(classes.shape[0],classes.shape[1], nspa)
