...
def viz_epoch(model, dataset, epoch, params, folder, writer=None):
    ...
    #todo if prox
    #real_samples, mask_real, real_lengths, labels, act_time_stamps = dataset.get_label_sample_batch(classes.numpy())
    #todo if humanact
    real_samples, mask_real, real_lengths = dataset.get_label_sample_batch(classes.numpy())
    act_time_stamps = None
    #classes = labels #todo enable if prox
    # to visualize directly

    print('the labels are: ', classes)
    ...
    if issingle:
        ...
        for key, val in generation.items():
            if len(generation[key].shape) == 1:
                ...
                if key == 'y':
                    generation[key] = val.reshape(nspa, nats, -1)
                else:
                    generation[key] = val.reshape(nspa, nats, *val.shape[1:])
