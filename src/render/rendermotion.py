...
from src.models.smpl import SMPLX, SMPL
...
def main():
    ...
    #filename = 'exps/prox/generation.npy'
    ...
    if 'prox' in filename:
        is_smplx = True
    if is_smplx:
        smpl_model = SMPLX(batch_size=1).eval().to("cpu")#TODO SMPL
        faces = smpl_model.body_model.faces
    else:
        smpl_model = SMPL(batch_size=1).eval().to("cpu")#TODO SMPL
        faces = None

    background = np.zeros((height, width, 3))
    renderer = get_renderer(width, height, faces)
