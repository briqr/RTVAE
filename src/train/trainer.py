...
def train_or_test(model, optimizer, iterator, device, mode="train"):
    ...
    with grad_env():
        for i, batch_i in tqdm(enumerate(iterator), desc="Computing batch"):
            ...
            batch = {key: val.to(device) for key, val in batch_i.items() if key!='frame_act_map'}
            if 'frame_act_map' in batch_i:
                batch['frame_act_map'] = batch_i['frame_act_map']
            ...
            #except:
            #   print('exception in batch processing')
            #   continue
