...
def calculate_diversity_multimodality(activations, labels, num_labels, seed=None):
    ...
    multimodality = 0
    label_quotas = torch.from_numpy(np.repeat(multimodality_times, num_labels)).to(labels.device)
    num_iters = 0
    while torch.any(label_quotas > 0) and num_iters < 100*label_quotas.sum():
        num_iters += 1
        # print(label_quotas)
        #first_idx = np.random.randint(0, num_motions)
        first_idx = torch.randint(0, num_motions,[1]).to(labels.device)[0]
        first_label = labels[first_idx]
        if not label_quotas[first_label]:
            continue

        second_idx = torch.randint(0, num_motions,[1]).to(labels.device)[0]
