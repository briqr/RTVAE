...
from src.recognition.models.PCNet import *
from src.recognition.models.utilitiesPC import *
...
def train_iter(input_tensor, seq_len,  model, optimizer, criterion_seq):
    ...
    mask = torch.zeros([len(input_tensor), seq_len]).to(device)
    for ith_batch in range(len(input_tensor)):
        mask[ith_batch, 0:seq_len] = 1
...
def evaluation(validation_loader, model, criterion_seq):
    ...
    for ind, batch in enumerate(validation_loader):
        seq_len = batch['x'].shape[3]
        input_tensor = batch['x'].to(device)
        input_tensor = input_tensor.permute(0,3,1,2).flatten(2)
...
def test_extract_hidden(model, data_train, data_eval):
    ...
    for ith, batch in enumerate(data_train):
        seq_len = batch['x'].shape[3]
        input_tensor = batch['x'].to(device)
        input_tensor = input_tensor.permute(0,3,1,2).flatten(2)
        label = batch['y']
...
#   evalloader = DataLoader(autodataset, batch_size=batch_size, shuffle=True)
  ...
  return np_out_train, label_train
...
def clustering_knn_acc(model, train_loader, eval_loader, criterion , num_epoches = 400, middle_size = 125):
    ...
    hi_train, label_train = test_extract_hidden(model, train_loader, eval_loader)
    #print(hi_train.shape)
    hi_eval = None
    label_eval = None

    lambda1 = lambda ith_epoch: 0.95 ** (ith_epoch // 50)
    np_out_train, au_l_train = train_autoencoder(hi_train, hi_eval, label_train,
                      label_eval, middle_size, criterion, lambda1, num_epoches)
...
def training(epoch, train_loader, eval_loader, print_every,
    ...
    for ith_epoch in range(1, epoch + 1):
        if ith_epoch % print_every == 0 or ith_epoch == 1:
            ...
            knn_acc_au = clustering_knn_acc(model, train_loader, eval_loader, criterion= auto_criterion)
            #knn_acc_au = 0
            print('%s (%d %d%%) TrainLoss %.4f KnnACC W/O-AEC: %.4f' % (
                timeSince(start, ith_epoch / epoch),
                ith_epoch, ith_epoch / epoch * 100, ave_loss_train, knn_acc_au))
        ...
        for it, batch in enumerate(train_loader): #(data, seq_len, label)
            seq_len = batch['x'].shape[3]
            input_tensor = batch['x'].to(device)
            input_tensor = input_tensor.permute(0,3,1,2).flatten(2)
