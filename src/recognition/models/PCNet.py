...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_tensor, seq_len):
        ...
        hidden = torch.empty((1, len(input_tensor), encoder_hidden.shape[-1])).to(device)
        count = 0
        for ith_len in range(len(input_tensor)):
            #hidden[0, count, :] = encoder_hidden[count, ith_len - 1, :]
            hidden[0, count, :] = encoder_hidden[count, seq_len - 1, :]
            count += 1
