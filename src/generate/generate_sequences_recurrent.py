...
def generate_actions(beta, model, dataset, epoch, params, folder, data_i, num_frames=60,
                     durationexp=False, vertstrans=True, onlygen=True, nspa=10, inter=False, writer=None):
    ...
    classes = labels
    #classes = torch.from_numpy(np.asarray([151, 47, 52])).unsqueeze(0)
    #classes = torch.from_numpy(np.asarray([151, 55])).unsqueeze(0)
    #classes = torch.from_numpy(np.asarray([122, 16, ])).unsqueeze(0)
    #classes = torch.from_numpy(np.asarray([9, 29 ])).unsqueeze(0) #dancing cross legs
    #classes = torch.from_numpy(np.asarray([12, 56])).unsqueeze(0)
    if len(classes) > 1 and np.unique(classes) == 1:
        return


    #if len(np.unique(classes)) <2 : #todo skip generating actions with less than 3 unique action labels
    #    return
    if not onlygen:
    ...
    for id, out in enumerate(output):
        ...
        if onlygen:
            ...
            out_type = sample_types[id]
        print('generating for sample %d'%data_i)
        for s in range(out.shape[0]):
            ...
            for l in range(len(all_verts)):
                ...
                vid_out.write(image)
            vid_out.release()
        #break # todo if only want to visualise the ground truth
...
def main():
    ...
    #fixseed(101167)    
    #for i in range(20, dataset.__len__()):
    #already_generated = defaultdict(lambda:0)
